...
This project uses Svelte Kit. To get started, run the following terminal commands:

- `npm install`
- `npm run dev`

You can then visit the app in-browser; it defaults to http://localhost:5173.
...
> `/static/assets`, and their metadata in `$lib/data.js`. Design
> tokens can be found in `tailwind.config.js`. The custom font is
> Inter by Rasmus Andersson.

## Troubleshooting

If you run into problems running a local development server, make sure you're running Node ^22 or consult [Svelte Kit docs](https://kit.svelte.dev/docs/).

Optionally you can switch using a version manager like [Volta](https://volta.sh/).
...
Use Flexbox to correctly align the elements within `$lib/components/SuperHeader.svelte`.
...
Inside `ShoeCard.svelte`, you'll find a `variant` variable you can use to figure out which flag, if any, needs to be rendered. It's up to you to create the flag, using tailwind-variants.
...
_NOTE:_ This exercise has minimal flexbox implications, and is mainly about revisiting lessons learned in the previous modules (including positioned layout and tailwindcss). Feel free to skip it if you'd prefer!
